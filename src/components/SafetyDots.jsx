...
export default function SafetyDots({ s, compact }) {
  ...
  const D = ({ c, t }) => <div style={{ display: "flex", alignItems: "center", gap: 5, minHeight: 20 }}><div style={{ width: 7, height: 7, borderRadius: "50%", background: c, boxShadow: `0 0 4px ${c}55`, flexShrink: 0 }} /><span style={{ fontSize: compact ? 10 : 12, color: c, fontFamily: "'DM Mono',monospace" }}>{t}</span></div>;
  return <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
    ...
    <D c={mc} t={compact
      ? `Safety margin: ${s.marginLabel.toLowerCase()}`
      : `Safety margin: ${s.marginLabel.toLowerCase()}`} />
    <D c={sc} t={compact
