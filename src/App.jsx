...
function cr(a,b){if(a===b)return null;return CO[[a,b].sort().join("+")]||CO[a+"+"+b]||CO[b+"+"+a]||null;}

// ── MECHANISM DATA — WHY each dangerous/unsafe combo is dangerous ──
// This is our enhancement beyond TripSit's color codes
const MECH={
...
"opioids+benzodiazepine":"The deadliest drug combination in America. Both suppress breathing through different mechanisms. Effects are multiplicative. Responsible for more overdose deaths than any other combo.",
...
"fentanyl+benzodiazepine":"The #2 deadliest drug combination. Fentanyl already has almost no safety margin — adding any benzo removes what little margin exists.",
...
"methamphetamine+tramadol":"Serotonin syndrome risk from tramadol's SNRI activity plus meth's serotonin release. Seizure risk compounded.",
"methamphetamine+pcp":"Extreme stimulation plus dissociation. Psychosis almost certain. Hyperthermia, violent behavior, death.",
"dxm+maoi":"DXM is serotonergic. MAOIs prevent serotonin breakdown. Fatal serotonin syndrome risk.",
"dxm+ssri":"Both increase serotonin. Serotonin syndrome risk — potentially fatal. Many people on SSRIs don't realize this interaction exists.",
"dxm+tramadol":"Both serotonergic. Serotonin syndrome plus respiratory depression plus seizures. Triple threat.",
"dxm+fentanyl":"Respiratory depression from fentanyl plus DXM's CNS depression. Some cough syrups have been found contaminated with fentanyl.",
...
"ketamine+opioids":"Both cause respiratory depression and unconsciousness. Aspiration risk extremely high.",
"ketamine+fentanyl":"Fentanyl contamination of ketamine has been reported. Both depress respiration. Fatal at combined doses that would be survivable alone.",
"ketamine+pcp":"Both NMDA antagonists. Effects stack unpredictably. Extreme dissociation, psychosis, and physical danger.",
"pcp+alcohol":"PCP makes you unaware of alcohol poisoning. Violent behavior plus complete disinhibition. Extremely dangerous.",
"pcp+opioids":"Respiratory depression plus PCP's unpredictable behavioral effects. Can't recognize OD symptoms.",
"pcp+fentanyl":"Respiratory depression masked by PCP's stimulant effects. Fatal before symptoms are recognized.",
"pcp+dxm":"Both NMDA antagonists. Effects compound unpredictably. Psychosis, hyperthermia, seizures.",
"pcp+maoi":"MAOIs potentiate PCP unpredictably. Extreme hypertension, psychosis, hyperthermia.",
"amphetamines+poppers":"Both affect cardiovascular system. Risk of dangerous arrhythmia and blood pressure instability.",
"methamphetamine+poppers":"Extreme cardiovascular strain from opposing vasodilation/vasoconstriction. Cardiac arrest risk.",
};
function getMech(a,b){return MECH[[a,b].sort().join("+")]||MECH[a+"+"+b]||MECH[b+"+"+a]||null;}
...
function Detail({s}){
  ...
  return <div>
    {/* Safety detail (the WHY behind the dots) */}
...
function Combos({selected}){if(selected.length<2)return null;const pairs=[];for(let i=0;i<selected.length;i++)for(let j=i+1;j<selected.length;j++){const a=S.find(x=>x.id===selected[i]),b=S.find(x=>x.id===selected[j]);pairs.push({a,b,risk:cr(selected[i],selected[j]),mech:getMech(selected[i],selected[j])});}
  ...
  return <div style={{marginBottom:18}}>
    ...
    {pairs.map(({a,b,risk,mech})=>{const lv=risk?RL[risk]:null;return <div key={`${a.id}-${b.id}`} style={{padding:"10px 12px",borderRadius:8,marginBottom:5,background:"rgba(255,255,255,0.02)",borderLeft:`3px solid ${lv?.c||"#555"}`}}>
      ...
      <div style={{display:"flex",gap:6,alignItems:"flex-start"}}><span style={{fontSize:16,color:lv?.c||"#666"}}>{lv?.i||"?"}</span><div><div style={{fontSize:11.5,fontWeight:600,color:lv?.c||"#888",fontFamily:"'DM Mono',monospace"}}>{lv?.l||"Unknown"}</div><p style={{margin:"2px 0 0",fontSize:11.5,color:"#7a7670",lineHeight:1.4}}>{lv?.d||"No data. Absence ≠ safety."}</p>
      {mech&&<p style={{margin:"6px 0 0",fontSize:12,color:lv?.c==="#ef4444"?"#e08080":lv?.c==="#f97316"?"#d09060":"#a0a090",lineHeight:1.5,background:"rgba(255,255,255,0.03)",padding:"6px 8px",borderRadius:5,borderLeft:`2px solid ${lv?.c||"#555"}44`}}><strong style={{fontFamily:"'DM Mono',monospace",fontSize:10.5}}>WHY:</strong> {mech}</p>}
      </div></div>
    </div>;})}
  </div>;
}
...
function Matrix(){
  ...
  return <div style={{background:"rgba(255,255,255,0.02)",borderRadius:12,border:"1px solid rgba(255,255,255,0.06)",padding:"18px 14px"}} onClick={e=>{if(e.target.tagName!=="TD")setSel(null);}}>
  ...
  {/* Detail — FIXED height box, always 88px, content swaps inside */}
  ...
  <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch",paddingBottom:8}}>
    <table style={{borderCollapse:"separate",borderSpacing:2,fontFamily:"'DM Mono',monospace"}}>
      <thead><tr>
        ...
        {S.map(s=> <th key={s.id} style={{color:CAT[s.cat].c,padding:"3px 1px",writingMode:"vertical-rl",textOrientation:"mixed",height:80,fontSize:10,fontWeight:500,whiteSpace:"nowrap"}}>{s.n.split("(")[0].trim().substring(0,12)}</th>)}
      </tr></thead>
      <tbody>{S.map(row=> <tr key={row.id}>
  ...
  {/* Legend — always here, never moves */}
...
function parseDur(s){const r=s.routes[0]?.dur||"";const m=r.match(/(\d+)\s*hr/g);if(m&&m.length){const nums=m.map(x=>parseInt(x));return Math.max(...nums);}const minM=r.match(/(\d+)\s*min/g);if(minM&&minM.length){const nums=minM.map(x=>parseInt(x));return Math.max(...nums)/60;}return 1;}
...
export default function App(){
  const[sel,setSel]=useState([]);const[view,setView]=useState("index");const[search,setSearch]=useState("");const[catF,setCatF]=useState(null);const[sort,setSort]=useState("default");const[expanded,setExpanded]=useState(null);
  const toggle=useCallback(id=>setSel(p=>p.includes(id)?p.filter(x=>x!==id):[...p,id]),[]);
  const filtered=useMemo(()=>{
    let list=S.filter(s=>{const ms=!search||s.n.toLowerCase().includes(search.toLowerCase())||s.aka.some(a=>a.toLowerCase().includes(search.toLowerCase()));return ms&&(!catF||s.cat===catF);});
  ...
  return <div style={{minHeight:"100vh",background:"#111113",color:"#c7c4be",fontFamily:"'Source Serif 4',Georgia,serif"}}>
    ...
    <main style={{maxWidth:1200,margin:"0 auto",padding:"14px 18px"}}>
      ...
      {/* ═══ INDEX — Full-width substance directory ═══ */}
      {view==="index"&&<div>
      ...
      {/* ═══ COMBINATIONS — Select substances + check combos ═══ */}
      {view==="combos"&&<div>
        ...
        {/* Substance selector — compact toggleable pills */}
      ...
      {/* ═══ SOURCES ═══ */}
      {view==="sources"&&<div style={{maxWidth:760}}>
        ...
        {[
        ...
        ].map((src,i)=> <div key={i} style={{background:"rgba(255,255,255,0.02)",borderRadius:10,border:"1px solid rgba(255,255,255,0.06)",padding:16,marginBottom:10}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,marginBottom:6,flexWrap:"wrap"}}>
            ...
            <span style={{fontSize:9.5,padding:"2px 8px",borderRadius:12,background:src.type==="Peer-reviewed"?"rgba(168,85,247,0.12)":src.type==="Government"?"rgba(59,130,246,0.12)":src.type==="Lab analysis"?"rgba(6,182,212,0.12)":"rgba(34,197,94,0.12)",color:src.type==="Peer-reviewed"?"#a855f7":src.type==="Government"?"#60a5fa":src.type==="Lab analysis"?"#06b6d4":"#22c55e",fontFamily:"'DM Mono',monospace",flexShrink:0}}>{src.type}</span>
          </div>
