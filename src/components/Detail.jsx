  ...
  return <div style={{ background: bad ? "rgba(239,68,68,0.07)" : safe ? "rgba(34,197,94,0.05)" : "rgba(0,0,0,0.2)", borderRadius: 10, padding: 14, border: `1px solid ${bad ? "rgba(239,68,68,0.25)" : safe ? "rgba(34,197,94,0.15)" : "rgba(255,255,255,0.06)"}`, marginBottom: 12 }}>
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 10 }}>
      <div style={{ width: 64, height: 64, borderRadius: 6, background: "rgba(255,255,255,0.02)", display: "flex", alignItems: "center", justifyContent: "center", border: "1px dashed rgba(255,255,255,0.08)", flexShrink: 0, position: "relative" }}>
        ...
        {s.lethal.cmp === "grain" && <div style={{ position: "absolute", bottom: 1, fontSize: 8, color: "#888", fontFamily: "'DM Mono',monospace" }}>← actual size</div>}
      </div>
      <p style={{ margin: 0, fontSize: 14, color: "#a09d97", lineHeight: 1.5, fontFamily: "'Source Serif 4',Georgia,serif" }}>{s.lethal.realworld}</p>
    </div>
    <p style={{ margin: 0, fontSize: 12, color: "#6b6860", lineHeight: 1.5, borderTop: "1px solid rgba(255,255,255,0.04)", paddingTop: 8 }}>{s.lethal.note}</p>
  </div>;
}
...
export default function Detail({ s }) {
  ...
  const Sec = ({ title, children }) => <div style={{ marginBottom: 14 }}><h4 style={{ margin: "0 0 6px", fontSize: 11, color: "#6b6860", fontFamily: "'DM Mono',monospace", textTransform: "uppercase", letterSpacing: "0.08em" }}>{title}</h4>{children}</div>;
  return <div>
    ...
    <Sec title="How much">{lvls.map((l, i) => <div key={l.l} style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 4 }}>
      <span style={{ width: 60, fontSize: 11, color: l.c, fontFamily: "'DM Mono',monospace", textAlign: "right" }}>{l.l}</span>
      ...
      <span style={{ fontSize: 12, color: "#c7c4be", fontFamily: "'DM Mono',monospace", minWidth: 75 }}>{l.v}</span>
    </div>)}</Sec>
    <LethalViz s={s} />
    {/* Responsive: stacks on mobile */}
    <Sec title="What it feels like"><div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>{s.feels.map(f => <span key={f} style={{ fontSize: 12, padding: "4px 9px", borderRadius: 5, background: "rgba(34,197,94,0.08)", color: "#5ab87a" }}>{f}</span>)}</div></Sec>
    <Sec title="What happens if you take too much"><div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>{s.odRisk.map(r => <span key={r} style={{ fontSize: 12, padding: "4px 9px", borderRadius: 5, background: "rgba(239,68,68,0.1)", color: "#e07070" }}>{r}</span>)}</div></Sec>
    <Sec title="What happens if you keep using it"><div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>{s.longTerm.map(l => <span key={l} style={{ fontSize: 12, padding: "4px 9px", borderRadius: 5, background: "rgba(99,102,241,0.08)", color: "#8b8fd0" }}>{l}</span>)}</div></Sec>
    <Sec title="Also called"><div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>{s.aka.map(a => <span key={a} style={{ fontSize: 12, padding: "4px 9px", borderRadius: 5, background: "rgba(255,255,255,0.04)", color: "#8a8780" }}>{a}</span>)}</div></Sec>
  </div>;
}
